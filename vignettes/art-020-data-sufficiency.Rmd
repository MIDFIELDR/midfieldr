...
set_fig_path("art-020-data-sufficiency-")
```
...
- Student A enters in Fall 1989 with a timely completion (TC) term of Spring 1995. 
- Student B enters in Fall 1991 with a TC term of Spring 1997. 
...
x <- c(1986, 1989, 1991, 1995, 1996, 1997)
...
If the input data frame has an existing column name matching any of the added column names, the existing columns are deleted. For example, having examined the details, you can delete them (especially useful in an interactive session) by running the same code with detail set to FALSE. 
...
Similar to `add_timely_term()`, this function has two required arguments and one optional `detail` argument that adds one extra column to show the institution's upper data limit. 
