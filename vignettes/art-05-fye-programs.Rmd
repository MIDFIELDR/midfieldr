...
2. Perform your own imputation. Use `prepare_fye()` to construct a data frame with appropriate predictor variables formatted such that it can be used as an input argument for the `mice()` function in the R package "mice" (multivariate imputation by chained equations). 
...
The main preparation work is done by `prepare_fye()`. The first argument of `prepare_fye()` is a data frame of all available  degree-seeking engineering students, with required variables student ID, race, and sex. Regardless of what programs we happen to be studying, if we impute starting majors for FYE students, we include all engineering programs at this point. 
...
This data frame is ready to use as the first argument of `prepare_fye()`. The remaining  arguments have default settings and so are optional. Arguments of `prepare_fye()`:
 
- `dframe` data frame with variables `mcid`, `race`, and  `sex`  
- `mdata`  term table, default `term` 
