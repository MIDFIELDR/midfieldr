...
vignette: >
  %\VignetteIndexEntry{Explore the workflow}
...
```{r echo=FALSE}
# used by my_session() 
pkg_names <- c("midfieldr", "midfielddata", "dplyr", "seplyr", "ggplot2")
# ggplot2::theme_set(theme_midfield())
```

We use `cip_filter()` to search the `cip` dataset first for "Engineering" and then filter the result for "Chemical", "Electrical", and "Industrial". We use the `drop_any` argument to omit technology programs and biochemical programs. 
...
Use `ever_filter()` to access the `midfieldterms` dataset and extract all students who ever enrolled in these programs. Use `race_sex_join()` to access the `midfieldstudents` dataset and append students' race and sex to the working data frame. 

```{r gather-ever}
students <- ever_filter(data = midfieldterms, codes = program_group_cip6) %>%
  race_sex_join() %>%
  print() # or View()
```
...
```{r grad}
graduated <- grad_filter(data = midfielddegrees, codes = program_group_cip6) %>%
  race_sex_join() %>%
...
For reference, the complete script for this example is collected below. We've omitted intermediate exploratory steps, retaining the minimum code required to obtain the results graph. 

```{r}
# complete vignette script
# TBD
```
