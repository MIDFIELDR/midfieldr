...
The primary input to `subset_feasible()` is a character vector of student IDs. To illustrate its use, we use the built-in data set `rep_ever` from the engineering case study as developed in the  *Compute the stickiness metric* vignette [(link)](compute_stickiness.html). 
...
## How `subset_feasible()` works

The remainder of the vignette describes the inner workings of  `subset_feasible()`, showing intermediate results at key stages. Its operation can be divided into 5 basic steps: 
