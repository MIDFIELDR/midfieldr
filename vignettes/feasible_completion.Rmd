...
Primary focus on midfieldr functions:

- `completion_feasible()` 

With an optional look at: 

- `get_status_degrees()` 
- `get_status_transfers()` 
- `get_institution_limits()` 
- `get_institution_hours_term()` 
- `term_addition()`
...
In the *Compute the stickiness metric* vignette [(link)](compute_stickiness.html), we obtain a character vector of student IDs of students ever enrolled in the programs of the engineering case study. These data are included with midfieldr as the built-in data set `exa_ever`. View its help page by loading midfieldr and running
...
This vector of IDs is the input to `completion_feasible()`, which performs the subsetting for completion feasibility.  
