...
`label_programs()` is used to create a data frame with 6-digit CIP codes and a user-supplied program label for grouping, summarizing, and joining. In this vignette we explore its arguments and continue the ongoing case study in four engineering programs.  

 

## Introduction 

The CIP subset for these programs is a built-in dataset `exa_cip`. We show how these data were obtained in the [vignette ("Explore program CIP codes")](explore_cip.html). 
...
For convenience, `exa_group` is included in midfieldr as a built-in data set. View its help page by running
