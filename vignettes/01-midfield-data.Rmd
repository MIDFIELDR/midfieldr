...
knitr::opts_chunk$set(fig.path = "../man/figures/midfield-data-")
...
The midfieldr package includes `cip`, a data frame of academic program names and codes based on the 2010 assignments curated by the US Department of Education. 
...
Loads with midfieldr. For definitions of variables, view the help page by running 
...
Loads with midfielddata. An observation describes course attributes keyed by student ID, term, and course. The record comprises `r obs_courses` observations of `r var_courses` variables and occupies approximately `r size_courses` memory. 
...
Loads with midfielddata. An observation describes term attributes keyed by student ID and term. The record comprises `r obs_terms` observations of `r var_terms` variables and occupies approximately `r size_terms` memory. 
...
Loads with midfielddata. Each observation describes student attributes when they complete a program, keyed by student ID. The record comprises `r obs_degrees` observations of `r var_degrees` variables and occupies approximately `r size_degrees` memory. 
