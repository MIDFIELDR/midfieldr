...
The function extracts all terms for all FYE students, including switchers, and  returns one row per student with columns for institution, race, sex, and proxy CIP code if any. 
...
**Filtering.** For imputing missing values we used all students ever enrolled in FYE, including switchers. In use, however, the proxies are substitutes for students starting in FYE. Therefore, we filter to retain the proxies of first-term FYE students only. 
