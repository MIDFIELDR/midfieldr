...
Part 1 of a development in two parts for working with engineering programs  illustrating our treatment of First-Year Engineering (FYE) programs when counting starters. Users working exclusively with either non-engineering programs or engineering at non-FYE institutions can disregard this vignette. 
...
2. [Using FYE proxies](art-071-using-fye-proxies.html). Incorporating a set of prepared FYE proxies with other first-term CIP codes to construct a data frame of starters. Suitable for multi-institutional (FYE and non-FYE) studies.  
...
: A student who changes majors. In creating FYE proxies, we are concerned with migration into or out of Engineering at the 2-digit level only (CIP 14). 

multiple imputation

: Method of imputing missing categorical data, in this case, imputing the 6-digit CIP codes of FYE proxies.
...
![](man/figures/art-070-creating-fye-proxies-pct-missing.png){width="100%"}
...
ggplot(DT, aes(x = pct_miss, y = reorder(cat_level, pct_miss))) +
