...
We join grouping variables to a bloc after initial processing (data sufficiency and degree seeking) and any other subsetting criteria that define a bloc. The two most common join operations are:

- *Program labels* &nbsp; using an inner join on CIPs. Inner joins are described in detail in [Degree seeking](art-030-degree-seeking.html).

- *Demographics* &nbsp; using a left join on IDs. Left joins are described in detail in [Blocs](art-050-blocs.html). 
