...
```{r setup, child = here::here("man/rmd/common-setup.Rmd")}
knitr::opts_chunk$set(fig.path = here::here("man/figures/art-000-"))
```
...
```{r child = here::here("man/rmd/define-student-level-data.Rmd")}
```
...
```{r child = here::here("man/rmd/note-practice-not-research.Rmd")}
```
...
```{r child = here::here("man/rmd/common-closing.Rmd")}
```
