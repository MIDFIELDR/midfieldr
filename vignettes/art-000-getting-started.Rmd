...
Contains one observation per student. Data are assumed to be current at the time the student was admitted to their institution. Navigate to data dictionary via `?student`. 
...
Contains one observation per student per course.  Navigate to data dictionary via `?course`.
...
Contains one observation per student per term. Navigate to data dictionary via `?term`.
...
Contains one observation per student per degree. Navigate to data dictionary via `?degree`.
