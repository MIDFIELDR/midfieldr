...
link-citations: yes
resource_files:
  - ../man/figures/art-06-fye-programs-fig1-1.png
  - ../man/figures/art-06-fye-programs-fig2-1.png
---
...
1. Use prepared predictions in `fye_start`, a built-in data set in midfieldr with predicted starting programs for all FYE students in the midfielddata practice data tables. 

2. Perform your own imputation. Use `condition_fye()` to construct a data frame with appropriate predictor variables formatted such that it can be used as an input argument for the `mice()` function in the R package "mice" (multivariate imputation by chained equations). 
...
We use two additional prepared data sets, `study_programs` with the case-study CIP codes and program names, and `study_students`, all students ever enrolled in these programs. To see their help pages, run 
...
rows_we_want <- term[, mcid] %chin% study_students[, mcid]
...
This code chunk above represents a routine we encounter fairly often: subset rows of one data frame to keep values of a key variable that match values of the same key variable in a second data frame.  Here, for example, we want to subset rows of `term` to keep values of `mcid` that match values of `mcid` in `study_students`. We can use `filter_by_key()` to accomplish the task. 

```{r}
# temporary object
try_this <- filter_by_key(term, 
                          match_to = study_students, 
                          key_col = "mcid", 
                          select = c("mcid", "term", "cip6"))

# compare to the DT we obtained above
all.equal(DT, try_this)
```

We'll use `filter_by_key()` for the remainder of the vignette for this type of subset task. Continuing the example, we set keys to organize the rows in block by ID and ordered terms. 
...
These data are subset again by matching program codes to those in `study_programs`, thereby omitting students who were not enrolled in one of the four programs in their first term. 
...
Now, turn to the FYE students. We subset `fye_start` for our four programs. Before using `filter_by_key()`, we change name of the key column from `start` to `cip6`. 


```{r}
# subset predicted FYE start programs 
setnames(fye_start, old = "start", new = "cip6")
study_fye <- filter_by_key(fye_start, 
                           match_to = study_programs, 
                           key_col = "cip6", 
                           select = c("mcid", "cip6"))
...
Adding program names from `study_programs` is helpful in interpreting the results. 
...
The main preparation work is done by `condition_fye()`. The first argument of `condition_fye()` is a data frame of all available  degree-seeking engineering students, with required variables student ID, race, and sex. Regardless of what programs we happen to be studying, if we impute starting majors for FYE students, we include all engineering programs at this point. 

To collect these students, we start by obtaining all CIP codes that start with "14" (Engineering).

```{r}
# get engineering CIP codes
engr_cip <- filter_text(cip, keep_text = "^14")
...
The students must all be degree-seeking, we want to subset rows of `engr` to keep values of `mcid` that match values of `mcid` in the `student` data table. We use `filter_by_key()` again. In this case, there is no change to the data frame because all students were degree-seeking. 

```{r}
# limit the population to degree-seeking students 
engr <- filter_by_key(engr,
                      match_to = student,
                      key_col = "mcid")
...
Lastly, we add demographic information with `add_race_sex()`. 

```{r}
# add race and sex variables 
engr <- add_race_sex(engr, student)

# examine the result
engr
```

We're starting with `r nrow(engr)` engineers  total. 

This data frame is ready to use as the first argument of `condition_fye()`. The remaining  arguments have default settings and so are optional. Arguments of `condition_fye()`:
 
- `dframe` data frame with variables `mcid`, `race`, and  `sex`  
- `midfield_table`  term table, default `term` 
- `fye_codes` CIP code(s) for FYE, default `"140102"` 

Using the default settings, 

```{r}
# set up a data frame for imputation 
fye <- condition_fye(engr, term)
...
Our first use of mice is to examine the framework by calling the function with zero iterations. (A warning may appear in your console. It can be ignored.)
...
engr <- filter_by_key(engr,
                      match_to = student,
                      key_col = "mcid")
engr <- add_race_sex(engr)
fye <- condition_fye(engr)
