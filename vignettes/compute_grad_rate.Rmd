---
title: "Compute the graduation rate matric"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Compute the graduation rate matric}
...
link-citations: yes
resource_files:
  - ../man/figures/vignette-compute-grad-rate-fig1-1.png
---
...
The IPEDS definition excludes students who attend college part-time, who transfer from one institution to another, and who change majors. The American Council on Education (ACE) estimates that this definition of graduation rate may exclude up to 60% of students at 4-year institutions [@Cook+Hartle:2011]. 
...
- `setnames()` rename columns 
- `DT[]` print a data frame
- `DT[rows_we_want]` subset rows
- `DT[, col := NULL]` remove a column 
- `DT[, col := FUN]` assign values in a column 
...
We use `filter_by_cip()` to extract from `midfieldstudents` the IDs of students matriculating in these programs.  
...
In the *Impute FYE starting programs* vignette [(link)](impute_fye.html), we derive the starting programs for all FYE students in midfielddata. Again, we filter by the program group CIP codes. 
...
The `starters` data frame is the input to `ipeds_subset()`.
...
The input to `ipeds_subset()` is the `starters` data frame with columns `id` and `start`. The function subsets the rows of the data frame per the IPEDS constraints and adds a new yes/no column `ipeds_grad`, where  "Y" indicates that the student's graduation accords with the IPEDS definition of graduation rate. 
...
The graduation rate data derived above is the data set `rep_grad_rate` in midfieldr.  View its help page by running
...
- *Impute FYE starting programs* vignette [(link)](impute_fye.html) 
- *Subset for completion feasibility* vignette [(link)](feasible_completion.html)  
- *Explore multiway data* vignette [(link)](explore_multiway.html) 
...
### A. How `ipeds_subset()` works

The first operation in `ipeds_subset()` is accessing `midfieldstudents` for entering term and transfer information for the students is `starters`. Then transfer students are removed, yielding: 
