...
link-citations: yes
resource_files:
  - ../man/figures/compute-grad-rate-fig1-1.png
---
...
knitr::opts_chunk$set(
  ...
  out.width = "70%",
  fig.align = "center",
  fig.path = "../man/figures/compute-grad-rate-"
)
...
The [Gather program data](gather_program_data.html) vignette describes how to obtain the program codes and names for these programs. Their  6-digit CIP codes and names are in the midfieldr data set `exa_program_group`. 

```{r}
# information saved from an earlier vignette
exa_program_group <- exa_program_group %>%
  print()
```

There are a total of `r length(unique(exa_program_group[["cip6"]]))` CIP codes for these programs. We have two columns from the `cip` dataset and the `program` column we added using `cip6_select()`. 
...
program_series <- exa_program_group[["cip6"]]
```

We use `start_filter()` to extract students from `exa_imputed_fye` data who start in  one of the programs in `program_series`. The `start_filter()` function has two main arguments:
...
fye <- start_filter(data = exa_imputed_fye, codes = program_series) %>%
  print()
```
...
Thus we have nearly 3900 FYE students imputed to have matriculated in one of our case study programs. 

We use `start_filter()` again on the `midfieldterms` data to obtain non-FYE starters in our programs. The `data` argument default value is `midfieldterms`, so the following two code chunks yield the same result. 

```{r}
# explicit value for the data argument
non_fye <- start_filter(data = midfieldterms, codes = program_series)

# implicit value for the data argument is midfieldterms
non_fye <- start_filter(codes = program_series)
...
We use `start_filter()` to collect all students starting in the programs we are studying. 

The `codes` argument in this case is an atomic vector of the 6-digit CIP codes in our `exa_program_group`. The function uses these codes to search `midfieldstudents`, the default data set. Use `print()` to check the result.  
...
```{r}
students <- start_filter(codes = program_series) %>%
  print()
```
...
```{r}
# should be a subset of the exa_program_group CIP we started with
...
started <- started %>%
  race_sex_join() %>%
  left_join(., exa_program_group, by = "cip6") %>%
