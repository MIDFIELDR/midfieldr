...
We use `start_filter()` to extract students from `case_fye` data who start in  one of the programs in `program_series`. The `start_filter()` function has two main arguments:
...
fye <- start_filter(data = case_fye, codes = program_series) %>%
        print()
```
