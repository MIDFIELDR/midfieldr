...
We use `start_filter()` to extract students from `cip_fye` data who start in  one of the programs in `program_series`. The `start_filter()` function has two main arguments:
...
fye <- start_filter(data = cip_fye, codes = program_series) %>%
  print()
```
