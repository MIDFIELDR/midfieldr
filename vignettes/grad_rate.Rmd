...
```{r echo=FALSE}
# used by my_session() 
pkg_names <- c("midfieldr", "midfielddata", "dplyr", "stringr", "seplyr")
```
 
The [Selecting and labeling programs](selecting_programs.html) vignette describes how to obtain the program codes and names for these programs. Their  6-digit CIP codes and names are in the midfieldr data set `case_program_group`. 
...
The prediction process is described in the vignette [Imputing starting majors for FYE students](imputing_fye.html). The results are available in the midfieldr data set `case_fye`. The data frame has two columns: the student ID of all FYE students in the midfielddata, and the 6-digit CIP of their imputed starting engineering major. 
...
fye <- starter_filter(data = case_fye, filter_by = program_series) %>%
  print()
```
...
non_fye <- starter_filter(data = midfieldterms, filter_by = program_series)

# implicit value for the data argument is midfieldterms
non_fye <- starter_filter(filter_by = program_series)
...
The `filter_by` argument in this case is an atomic vector of the 6-digit CIP codes in our `program_group`. The function uses these codes to search `midfieldstudents`, the default data set. Use `print()` to check the result.  
...
```{r}
students <- starter_filter(filter_by = program_series) %>%
  print()
```
...
For the data in this package, the imputed values are saved in the `case_fye` data set. Use `starter_filter()` again with `case_fye` as the `data` argument. 
...
graduated <- grad_filter(filter_by = program_series) %>%
  print()
```
