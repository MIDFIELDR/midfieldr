...
We use `start_filter()` to collect all students starting in the programs we are studying. `start_filter()` has one required argument:
...
```{r}
students <- start_filter(series = program_series) %.>%
  glimpse(.)
```
...
For the data in this package, the imputed values are saved in the `case_fye` data set. Use `start_filter()` again with `case_fye` as the optional `reference` argument. 

```{r}
fye <- start_filter(series = program_series, reference = case_fye) %.>%
  glimpse(.)
```
...
Next we gather the graduates in out program group, but restrict ourselves to students who graduate in the same program in which they started. 
