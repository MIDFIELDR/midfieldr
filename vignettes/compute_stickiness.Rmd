...
The [Gather program data](gather_program_data.html) vignette describes the  process for obtaining the program codes and names for four engineering programs: Civil, Electrical, Industrial, and Mechanical. The output of that vignette is saved as a midfieldr case data set, `case_program_group`. You may 
...
The `codes` argument in this case is an atomic vector of the 6-digit CIP codes in our `program_group`. The function uses these codes to search `midfieldterms`, the default reference data set. Use `print()` to check the result. 
