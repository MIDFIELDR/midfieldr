...
In the *Gather programs* vignette [(link)](get_programs.html), we create the program group of 6-digit codes and user-defined program names  for the engineering case study. These data are included with midfieldr as the built-in data set `exa_group`. View its help page by running
...
For convenience, the IDs of these enrollees are included in midfieldr as the built-in character vector `exa_ever`. View its help page by running
...
For convenience, the IDs of these graduates are included in midfieldr as the built-in character vector `exa_grad`. View its help page by running
...
For convenience, this data frame is included in midfieldr as the built-in data set `exa_stickiness`. View its help page by running
...
In the *Explore multiway data* vignette [(link)](explore_multiway.html), we describe this transformation in detail. The transformed multiway data are included with midfieldr as the built-in data set `exa_stickiness_mw`. View its help page by running
...
To see that the categorical variables are factors and not characters: 
...
To view the levels of the two categories in order of increasing median stickiness:  
...
We use conventional ggplot2 functions to graph the multiway. 
