---
title: "Compute stickiness"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Compute stickiness}
...
knitr::opts_chunk$set(
  ...
  out.width = "70%",
  fig.align = "center",
  fig.path = "../man/figures/compute-stickiness-"
)
...
Longitudinal "stickiness" is the ratio of the number of students graduating in a program to the number of students ever enrolled in that program [@Ohland+Orr+etal:2012]. 
...
The [Gather program data](gather_program_data.html) vignette describes the  process for obtaining the program codes and names for four engineering programs: Civil, Electrical, Industrial, and Mechanical. The output of that vignette is saved as a midfieldr case data set, `exa_group`. You may 

- create the program group by running the code in the vignette 
- or load the saved case data 

The case data are loaded with midfieldr. Simply assign the data to an object  such as `exa_group`.  
...
Examine the result: there are a total of `r length(unique(exa_group[["cip6"]]))` CIP codes for these programs. We have  two columns from the `cip` dataset and the `program` column we added using `cip6_select()`. 
...
We use `get_enrollees()` to collect all students ever enrolled in the programs we are studying. `get_enrollees()` has two required arguments:

- `data` data frame of term information, default `midfieldterms` 
- `codes` atomic character vector of 6-digit CIP codes

The `codes` argument in this case is an atomic vector of the 6-digit CIP codes in our `exa_group`. The function uses these codes to search `midfieldterms`, the default reference data set. Use `print()` to check the result. 

```{r}
# extract the 6-digit CIPs as a search series
program_series <- exa_group[["cip6"]] %>%
  unique() %>%
  sort() %>%
  print()
```
...
```{r}
students <- get_enrollees(codes = program_series) %>%
  print()
```
...
`race_sex_join()` accepts any data frame with the MIDFIELD student `id` variable and joins the student race and sex from the `midfieldstudents` reference data to these data. If using the default reference data, the `demographics` argument can be omitted. 

```{r}
students <- race_sex_join(data = students, demographics = midfieldstudents) %>%
  print()
```
...
Next we join our custom program names to the student data. We use `left_join()` (a dplyr function) to join `exa_group` to `students` by the 6-digit CIP code. 

```{r}
students <- left_join(students, exa_group, by = "cip6") %>%
  print()
```
...
`get_graduates()` extracts a subset of the `midfielddegrees` data containing the  unique students ever graduating from the programs listed in `program_series `. 

The procedure for gathering graduates is nearly identical to the procedure for gathering the ever-enrolled. We use `get_graduates()` to search the  `midfielddegrees` dataset using the same `codes` argument; we join race, sex, and program in the same way; and when we count students, the totals are assigned to the new variable `grad`.
...
graduated <- get_graduates(codes = program_series) %>%
  race_sex_join() %>%
  left_join(exa_group, by = "cip6") %>%
