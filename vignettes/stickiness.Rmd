...
# make this fix to the data package
midfieldstudents <- rename(midfieldstudents, CIP6 = program)

library(midfieldterms)

# make this fix to the data package
midfieldterms <- rename(midfieldterms, CIP6 = program)
```
...
`count_and_fill()` counts the number of students grouped by the variables we just selected. If race or sex are in the list, empty entries are assigned a zero. 
...
In summary, four function calls are needed to gather and count the numbers of students ever enrolled in a set of programs. We can chain those functions together in one code chunk as follows: 
...
Alternatively, suppose we want to group by program and sex, but ignore race. We duplicate the earlier code chunk, but edit the `select()` arguments. 

```{r}
# group by program and sex
ever_enrolled <- gather_ever(midfieldterms, study_group) %>%
  join_demographics() %>%
  select(program, sex) %>%
  count_and_fill(., "ever")

# this set is small enough to view in a table
