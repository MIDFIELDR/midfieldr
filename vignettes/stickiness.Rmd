...
Examine the result: there are a total of 12 CIP codes for these programs. Our `program` labels agree with the default `cip6name` values. 
...
This example has 4 programs, 2 sexes, and 8 race values, so we have 64 possible combinations of program, sex, and race, yet our result above has only 63 rows, not the expected 64. This outcome tells us that one of the possible combinations is missing.
