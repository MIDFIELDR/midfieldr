...
We use `gather_ever()` to collect all students ever enrolled in the programs we are studying. `gather_ever()` has one required argument:

- `series` atomic character vector of 6-digit CIP codes

The `series` argument in this case is an atomic vector of the 6-digit CIP codes in our `program_group`. The function uses these codes to search `midfieldterms`, the default reference data set. 
