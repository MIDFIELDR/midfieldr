...
cve <- cip_filter(series = "^1408")
ele <- cip_filter(series = "^1410")
mce <- cip_filter(series = "^1419")
ise <- cip_filter(series = "^1435")
```
...
```{r}
programs_to_study <- bind_rows(cve, ele, mce, ise)
```

Examine the result: there are a total of `r length(uniques(programs_to_study[["cip6"]]))` CIP codes for these programs. We have 7 columns: 6 from the `cip` dataset and the one `program` column of labels we added using `cip_label()`. 
...
When searching the midfielddata datasets for students in these programs, our search functions use the 6-digit CIP codes in the `cip6` variable. To confirm that the search terms are what we expect, we can extract and view the `r length(uniques(programs_to_study[["cip6"]]))` unique codes. 

```{r}
unique(programs_to_study[["cip6"]])
```
...
students <- gather_ever(programs_to_study)
```
...
`gather_grad()` extracts a subset of the `midfielddegrees` data containing the  unique students ever graduating from the programs listed in `programs_to_study ` and joins our user-defined `program` labels to these data. 
...
graduated <- gather_grad(programs_to_study) %>%
  race_sex_join() %>%
...
`complete_se()` turns the implicit missing values into explicit missing values, and we can fill the missing counts of `ever`  and `grad` with zero. The completed data frame has the expected `r n_comb` observations. 

```{r}
# fill implicit missing values
stickiness <- stickiness %>%
  complete_se(grouping_variables, fill = list(ever = 0, grad = 0))

stickiness
```

Finally, we create `stick`, the ratio of `grad` to `ever`. Stickiness is reported as a fraction between 0 and 1, or `NaN` if the ever count for a group is zero. 
...
# count students ever enrolled, by group
ever_enrolled <- gather_ever(programs_to_study) %>%
	race_sex_join() %>%
	group_summarize(grouping_variables, ever = n())

# count students graduating, by group
graduated <- gather_grad(programs_to_study) %>%
	race_sex_join() %>%
	group_summarize(grouping_variables, grad = n())
...
stickiness <- left_join(ever_enrolled, graduated, by = grouping_variables) %>%
	ccomplete_se(grouping_variables, fill = list(ever = 0, grad = 0))
mutate(stick = round(grad / ever, 3))
...
# graph
base <- ggplot(stickiness_multiway, aes(x = stick, y = race_sex)) +
	facet_wrap(~program, ncol = 1, as.table = FALSE) +
	geom_point(na.rm = TRUE) +
	labs(x = "Stickiness", y = "") +
	theme_midfield()
...
base +
	geom_point(NA_data, aes(label = "N",
								x = stick, 
								y = race_sex),
						hjust = 0, 
						vjust = 0)
