...
set1 <- cip_filter(series = "^1408") %>% 
	add_column(program = "Civil Engineering")
set2 <- cip_filter(series = "^1410") %>% 
	add_column(program = "Electrical Engineering")
set3 <- cip_filter(series = "^1419") %>% 
	add_column(program = "Mechanical Engineering")
set4 <- cip_filter(series = "^1435") %>% 
...
```{r}
# bind the programs
program_group <- bind_rows(set1, set2, set3, set4) %>%
