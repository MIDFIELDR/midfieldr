...
ggplot(mw, aes(x = stick, y = race_sex)) +
...
# create the dual multiway graph
ggplot(mw, aes(x = stick, y = program)) +
...
The `order_by` argument in `condition_multiway()` provides options for ordering the two categorical variables (panel order and row order) in a multiway graph. The following values are possible for `order_by`: "percent", "median" (default), "mean", "sum", and "alphabet." 

Many components of the following graphs are identical. To reduce duplication we set up a graphical template with a default data object (`DT`). To use the template, we use a replacement operator `%+%` and write `p %+% new_data` to substitute `new_data` for the default data set in `p`. 

```{r}
# create a common graph framework to use in the examples below
p <- ggplot(DT, aes(x = stick, y = race_sex)) +
  facet_wrap(vars(program), ncol = 1, as.table = FALSE) +
  geom_point(na.rm = TRUE) +
  labs(
...
`order_by = "median"`  
: (default) Orders by the median of values in the quantitative column grouped by category.
...
: In this example, the quantitative variable is no longer stickiness so we create a new graph instead of using the template. 
...
ggplot(dtf, aes(x = ever, y = race_sex)) +
  facet_wrap(vars(program), ncol = 1, as.table = FALSE) +
  geom_point(na.rm = TRUE) +
