...
resource_files:
  ...
  - ../man/figures/vignette-explore-multiway-fig2-1.png
---
...
`order_multiway()` is used to transform multiway data such that the two categorical variables are factors with levels ordered by the third, quantitative variable. In this vignette we explore its arguments using a case study in four engineering programs.
...
In the *Compute the stickiness metric* vignette [(link)](compute_stickiness.html), we construct the data frame with the numbers of students ever enrolled in the case study programs, the number graduating, and the resulting program stickiness grouped by race/ethnicity, sex, and program. These data are included with midfieldr as the built-in data set `exa_stickiness`. View its help page by loading midfieldr and running
...
For convenience, these multiway data are included in midfieldr as the built-in data set `exa_stickiness_mw`. View its help page by running
