...
We'll use the [stickiness metric](compute_stickiness.html) to illustrate multiway graphs and data tables. Stickiness is the ratio of the number of students graduating in a program to the number of students ever enrolled in that program [@stickiness2012]. 
...
```{r}
stickiness_table <- exa_stickiness %>%
...
stickiness_table <- stickiness_table %>%
  ...
  select("Program" = program, "Race" = race, "Sex" = sex, "Stickiness" = stick) %>%
