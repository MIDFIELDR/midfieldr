...
**Copying.** Copy the data file `study_observations` containing the final set of case study "ever" and "grad" observations as developed in the previous segment. These data are included with midfieldr.
...
In this form, the data are included with midfieldr as the `study_results` data set. You can view the help page (and data dictionary) by running
...
**Write to file.** Results like those just prior to the dissemination preparation step are often written to file as an intermediate result. In this case, the pre-dissemination data frame is included with midfieldr as `study_results`.
