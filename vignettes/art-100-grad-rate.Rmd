...
*Data sufficiency.* Obtain the IDs of all students for whom the data sufficiency criteria are satisfied. Code reproduced from [Data sufficiency](art-020-data-sufficiency.html).
...
*Create start variable.* Isolate the starting term and create the `start` variable. Code reproduced from [Starters](art-070-starters.html). 
...
*Filter by program.* We join the case study programs labels to the data frame and filter to retain the desired programs. Code reproduced from [Starters](art-070-starters.html). 
...
*Create variables.* Add a group column with value `start` throughout to distinguish these observations from the observations of starters we derived earlier. 
...
*Copy.* Prevents operations on `DT` from affecting `starters` by reference  [@data.table-reference-semantics].
...
*Select.* We can start with the IDs only. 
...
*Add variables*. Add the degree information using a left-outer join with `degree`. The CIP code is the program (if any) from which the student graduated. 
...
*Filter by program.* Like we did for starters earlier, we join the case study program labels to the data frame and filter to retain the desired programs. Code reproduced from [Starters](art-070-starters.html). 
...
*Completion status.* Add columns for timely completion term and completion status. Filter to retain observations for which completion is timely. Code reproduced from [Completion status](art-080-graduates.html). 
...
*Preparation.* &nbsp; The data frame we preserved earlier is the intake for the reusable code chunk below.  
