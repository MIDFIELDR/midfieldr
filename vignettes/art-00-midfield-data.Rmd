...
While we suggest installing and loading the data.table package, we use base R functions in the vignettes to reduce the cognitive load on new users. We attempt to introduce data.table syntax only when there is a clear benefit to the reader. 
...
Some of the code snippets in the midfieldr vignettes are written with relative file paths that assume the existence of this minimal structure. Use any organization that suits your needs---I have found a slightly more detailed structure to be useful for many of my data visualization projects:

```
/project-name
        /data       # processed versions of raw data
        /data-raw   # raw data, read-only, can also include scripts for basic cleaning 
        /manage     # project management, proposals, budgets, anything from collaborators  
        /reports    # reproducible reports, R markdown files, bibliographies, CSL files, etc  
        /results    # reproducible results:  graphs and tabulated data   
        /scripts    # R files that convert data into results
        LICENSE     # explicit licenses for text, code, and data 
