...
We add the column `end_fye` to the `all_fye_terms` data frame and filter to keep only later terms, i.e., `term > end_fye`. Arranging by ID and *ascending*  term and grouped by ID, each first row is a student's first term after FYE. Because this data frame denotes students transitioning to an engineering major, we filter to keep only those whose CIP starts with "14." 
...
```{r}
mi_output_complete <- complete(mi_output)
case_fye <- mi_output_complete %>%
  select(id, start) %>%
...
md.pattern(mi_output_complete, plot = TRUE)
```
