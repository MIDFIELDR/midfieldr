...
resource_files: 
  ...
  - ../man/figures/art-04-data-sufficiency-fig2-1.png
---
...
- Start with the case study programs (`study_programs`) 
...
- Result is set of students for further analysis (`study_students`)
...
- `details`, when TRUE, provides additional columns in the output that were used to determine the results in `timely_term`. 
...
We can create a simple dot chart to compare the numbers of students in these programs. 
...
Before graphing, we convert the program variable to a factor and order the factor by `N` to automatically order the rows by `N` instead of alphabetically. 
