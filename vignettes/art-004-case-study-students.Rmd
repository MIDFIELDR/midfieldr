...
set_fig_path("art-004-case-study-students-")
```
...
In general, the `term` data can include non-degree-seeking students. To limit the study to degree-seeking students only, we subset rows of `DT` with `mcid` values that match the `mcid` values in the `student` table (all students in the `student` data table are degree-seeking). As it happens, in this example all students are degree-seeking, so the results don't change. 
...
The midfieldr function `add_data_sufficiency` finds the upper limit of their institution's data range, compares it to the timely completion term, and reports in a TRUE/FALSE column whether the data are sufficient. 
