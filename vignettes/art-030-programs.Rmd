...
In the US, instructional programs are encoded by 6-digit numbers curated by the US Department of Education. The US standard encoding format is a two-digit number followed by a period, followed by a four-digit number, for example, 14.0102. 

MIDFIELD uses the same numerals, but omits the period, i.e., 140102, and stores the variable as a  character string. 
...
*Prepared data.* `cip`, included with midfieldr, contains codes and names for 1582 programs based on the 2010 CIP [@NCES:2010]. 
...
Subset a data frame, `cip` for example, retaining rows that match or partially match a vector of character strings.

*Arguments*

- **`dframe`** &nbsp; Data frame to be subset, usually `cip`.
...
*Output.* &nbsp; Subset of `dframe` with rows matching elements of `keep_text`. Additional subsetting if optional arguments specified. 
...
Filtering the CIP data for all programs containing the word "engineering" yields `r nrow(filter_search(cip, "engineering"))` observations. 
...
To construct the data frame of CIPs and program names in the case study, we could use any of the methods described above. Here, we assume we know the 4-digit CIPs of our case study programs and label them in a minimum number of steps. 
...
*Verify built-in data.* To avoid deriving this data frame each time it is needed in other vignettes, the same information is provided in the `study_programs` data frame included with midfieldr. Here we verify that the two data frames are identical. 
