...
The CIP data is included with midfieldr. 
...
The variables in `cip` are all characters; `filter_search()` subsets `cip` (or any data frame) using character search terms. You can view the help page by running
...
: Data frame to be subset, usually `cip`. 

`keep_text` 

: Character vector of search text for retaining rows, not case-sensitive 

`drop_text` 

: Optional character vector of search text for dropping rows

`select` 

: Optional character vector of column names to search and return, default all columns 
...
This data frame is included with midfieldr as the `study_programs` data set. You can view the help page (and data dictionary) by running
...
# Verify results
all.equal(four_programs, study_programs)
```
