...
The first two argument names can be omitted if they are in the correct order. We will generally omit argument names for the first one or two arguments unless disallowed by the function. 
...
Again, for the R novice who prefers to name each argument, the same result is obtained with this code chunk: 
...
cip_filter(cip, kseq(540101, 540199, 1)) %>%
        print()
```

```{r echo = FALSE}
kable2html(cip_filter(cip, kseq(540101, 540199, 1)))
```
