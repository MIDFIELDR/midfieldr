...
Instructional programs in the US are encoded by "CIP" codes. CIP is the acronym for *Classification of Instructional Programs*, a taxonomy of academic programs curated by the US Department of Education [@NCES:2010].  
...
We explore the `cip` data set using the `filter_by_text()` function. We illustrate several search strategies and we conclude by constructing an example data set used in subsequent vignettes. 
...
Examine the values in the data set. 
...
- one grouping  at the 2-digit level (code 41)  
- `r length(unique(df$cip4))` groups at the 4-digit level (codes 4100, 4101, ...)
...
`filter_by_text()` filters a data frame using character search terms. The arguments are: 
...
For example, filtering the CIP data for all programs containing the word "engineering" yields `r nrow(filter_by_text(cip, "engineering"))` observations. 
...
filter_by_text(cip, keep_any = c(050125, 160501))
```
...
```{r error = TRUE, purl = FALSE}
filter_by_text(cip, keep_any = c("111111", "Bogus", "^55"))
```
