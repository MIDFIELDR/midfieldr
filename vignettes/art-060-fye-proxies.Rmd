...
# figures
knitr::opts_chunk$set(
  fig.path = "../man/figures/art-060-fye-proxies-",
  fig.width = 6,
  fig.asp = 1 / 1.6,
  out.width = "70%",
  fig.align = "center"
...
This is one case in which data sufficiency and degree seeking filters are not used. 
...
The vertical dashed line indicates the `r m_imputations`% percent missing data overall. 

![Figure1: Percent missing data by category.](../man/figures/art-060-fye-proxies-pct-missing.png){width="100%"}
...
The output of this code chunk is the result reproduced in [Reusable code]. 
...
proxy_program_names <- filter_cip(cip, keep_text = proxy_cips)
...
## Reusable code: FYE proxies

*Preparation.* &nbsp; The `student` and `term` data tables are the intakes for this section. 

```{r}
data(student, term, package = "midfielddata")
```

*FYE proxies.* &nbsp; A summary code chunk for ready reference. Requires editing if one is using source data other than the MIDFIELD practice data or predictor variables other than institution, race/ethnicity, and sex. The number of imputations (*m*) depends on the percentage of missing data. The random number seed is set only for reproducibility. 
...
# Percentage missing data suggests number of imputations
N_impute <- sum(is.na(DT$proxy))
N_complete <- sum(complete.cases(DT[, .(mcid, race, sex, institution)]))
m_imputations <- round(100 * N_impute / N_complete, 0)
...
DT <- mice::complete(DT_mids)
setDT(DT)
DT <- DT[, .(mcid, proxy = as.character(proxy))]
...
*Verify built-in data.* &nbsp;  To avoid deriving this data frame each time it is needed in other vignettes, the same information is provided in the `fye_proxy` data frame included with midfieldr. Here we verify that the two data frames have the same content. 
...
ggsave(
  filename = "man/figures/art-060-fye-proxies-pct-missing.png",
  plot = p,
  width = 7,
  height = 0.4 * 7,
  units = "in"
