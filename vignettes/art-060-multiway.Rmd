...
vignette: >
  %\VignetteIndexEntry{Mutliway data and charts}
...
View the data set help page (and data dictionary) by running
...
`condition_multiway()` converts the categorical variables to factors ordered by the quantitative variable. View the help page by running

    ? condition_multiway()

Its arguments are

- `dframe`: data frame that includes three columns of multiway data
- `x`: name (in quotes) of a single quantitative variable
- `yy`: vector of names (in quotes) of exactly two categorical variables
- `order_by`: "median" (default) or "percent" to assign the method of ordering the categories 
- `param_col`: vector of two names (in quotes) of the columns used in the "percent" ordering method 
