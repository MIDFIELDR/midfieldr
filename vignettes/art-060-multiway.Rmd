...
We prepare the data for use as input to `order_multiway_categories()` and use the results to construct multiway charts ordered by category median values and by category percentage values. 
...
Before we apply the `order_multiway_categories()` function, we edit the categorical variables to create the forms we want in the final charts or tables.  
...
For this example, we select the count of graduates (`grad`) as our quantitative variable and use `order_multiway_categories()` to order the categories by median numbers of graduates. 
...
Applying `order_multiway_categories()`, we specify `"grad"` as the quantitative column, `"program"` and `"people"` as the two categorical columns, and `"median"` as the method of ordering levels.

```{r}
# Convert categories to factors ordered by median
DT_count <- order_multiway_categories(DT_count,
  quantity = "grad",
...
```{r}
#| collapse: true
# Verify order_multiway_categories() output
...
Because the `method` argument has a default value of "median", the following two code chunks produce identical results. 

```{r}
#| collapse: true
# Using explicit median method
p <- order_multiway_categories(DT_count,
  quantity = "grad",
  categories = c("program", "people"),
  method = "median"
)

# Using implicit median method
q <- order_multiway_categories(DT_count,
  quantity = "grad",
  categories = c("program", "people")
)
...
Because stickiness is a ratio, we set `method` to "percent" and assign `grad` and `ever` to the `ratio_of` argument. `order_multiway_categories()` then sums the `ever` and `grad` counts by category and produces grouped percentages to order the category levels. 

```{r}
# Convert categories to factors ordered by group percentages
DT_ratio <- order_multiway_categories(DT_ratio,
  quantity = "stick",
  categories = c("program", "people"),
  method = "percent",
