...
We'll use the [stickiness metric](stickiness.html) to illustrate multiway graphs and tables. Stickiness is the ratio of the number of students graduating in a program to the number of students ever enrolled in that program [@stickiness2012]. 
...
Optionally, using the `medians` argument, the function returns the median values used in ordering the factors. By arranging the rows of the data frame by `program` and `race_sex` factors, we can see that they are ordered by the medians shown. 

```{r}
# return the median stickiness value for both categories
df <- multiway_order(stickiness, medians = TRUE) %>%
