...
```{r echo = FALSE}
cip_ag <- cip_filter(cip, series = "^0101")
```

At the 6-digit level, each of the `r nrow(cip)` rows is a unique program.
...
Alternatively, only the `except` argument must be named, so the following yields the same result, 
...
```{r echo = FALSE}
# using kable_styling() for output but conceal from novice user
kable2html(cip_filter(cip, "civil") %>% 
  cip_filter(., "engineering") %>% 
  cip_filter(., except = "technology"))
```
