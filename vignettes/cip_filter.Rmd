---
title: "Exploring program CIP codes"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Exploring program CIP codes}
...
[Selecting the 6-digit codes] <<<<< needs work     
[References]    
[Session information]    
...
```{r echo=FALSE}
# for session_info
package_string <- "midfieldr|dplyr"
```
...
```{r}
cip_filter(data = cip, series = "civil") %>%
...
kable2html(cip_filter(data = cip, series = "civil"))
```
...
```{r results = "hide"}
cip %>%  
  cip_filter(data = ., series = "civil") %>% 
  cip_filter(data = ., series = "engineering") %>% 
  cip_filter(data = ., except = "technology") %>% 
  print()
```

Only the `except` argument must be named, so the following code chunk yields the same result, 
...
```{r results = "hide"}
cip_filter(data = cip, series = "civil engineering", except = "technology") %>% 
  print()
```

```{r echo = FALSE}
kable2html(cip_filter(data = cip, series = "civil engineering", except = "technology"))
```
...
```{r results = "hide"}
cip_filter(data = cip, series = "german") %>%
  print()
```

```{r echo = FALSE}
kable2html(cip_filter(data = cip, series = "german"))
```
...
Note that the series argument is a vector. 
...
Once we've found the codes for the programs we want to study, we select the 6-digit codes 
