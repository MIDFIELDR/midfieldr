...
- Practice read/write to `data` directory 
...
We save the `case_study` data frame as a CSV file to the `data` directory for easy access as we work through the study. Here we use fast read and write `fread()` and `fwrite()`  from data.table. You may prefer other read/write functions such as those in base R or packages readr or rio. 
