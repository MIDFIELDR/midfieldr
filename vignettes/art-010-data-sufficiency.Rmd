...
![](../man/figures/art-010-data-sufficiency-upper.png){width="100%"}
...
![](../man/figures/art-010-data-sufficiency-lower.png){width="100%"}
...
*Load source data.* MIDFIELD practice data tables as described in [Getting started](art-000-getting-started.html). Of the four MIDFIELD data tables, `course` and `term` are the most inclusive. The smaller of the two, `term`, is often selected for initial processing.
...
A convenience function to reduce the dimensions of a MIDFIELD data table at the start of a session by selecting only those columns typically required by other midfieldr functions. Operates on a data frame to retain columns having names that match or partially match search terms. Rows are unaffected. 

Using this function is optional. The primary benefit is reducing screen clutter when viewing data frames during an interactive session. The disadvantage is that the deleted columns are unavailable unless you first set aside a copy of the source file or reload it using `data()` when you need it. 

*Arguments*

- **`dframe`** &nbsp; MIDFIELD data frame, typically `student`, `term`, or `degree`. 

- **`select`** &nbsp; Optional character vector of column names to retain, default `c("mcid", "institution", "race", "sex", "^term", "cip6", "level")`. 
...
In an interactive session, this result is much less cluttered than the original source file. 
...
As just shown, the data frame has `r nrow(DT)` observations. We will usually note the number of observations as they change.

*Selecting.* We start with the ID column only.
...
*Filtering.* Duplicate rows are present because term data has one row per student per term. We filter for unique IDs.
...
Let's examine two specific observations to illustrate.
...
# Using source data named something else
...
# Obtain data sufficiency and supporting variables
...
Let's examine three specific observations to illustrate. In these excerpts, the data range for the institution is Fall 1987 to Summer 2003.
...
Collecting in one code chunk the essential data sufficiency elements and retaining only those observations for whom the result is "include". 
...
We use `add_timely_term()` to add a variable with a student's estimated timely completion term. We use `add_data_sufficiency()` to add a variable that labels each observation for inclusion or exclusion based on data sufficiency at the upper and lower limits of an institution's data range. 
