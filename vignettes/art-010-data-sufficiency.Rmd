...
Starting with a data frame of Student Unit Record (SUR) observations (like `DT` above) that includes an ID variable, we add a timely completion term column using `add_timely_term()`. View the help page by running
...
Continuing with a data frame of SUR observations, we add the data sufficiency column using `add_data_sufficiency()`. View the help page by running
