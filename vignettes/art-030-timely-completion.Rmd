...
set_fig_path("art-030-timely-completion-")
```
...
? add_completion_timely
```
This function has two required arguments and one optional `detail` argument. The input `dframe` must have a `timely_term` column.  In this case, `detail` adds three extra columns to show whether a program was completed, and if so, the term and degree CIP. 
...
If we start with the result from above and add detail, 
...
```{r}
# Improve viewing
cols_we_want <- c("program", "cip6", "completion", "completion_timely", "status")
