...
```{r echo=FALSE}
# for session_info
package_string <- "midfieldr|dplyr|stringr"
```
...
cip_filter(data = cip, series = "engineering", except = "technology") %>%
...
```{r results = "hide"}
cip %>% 
  cip_filter(data = ., series = cip_math_stat) %>% 
...
sel      <- stringr::str_detect(x$packages$package, package_string)
plat     <- x$platform
pkgs     <- x$packages[sel, ]
pkgs[11] <- NULL 
