...
cip_filter(series = "Engineering") %>%
  cip_filter(series = c("Civil", "Electrical", "Industrial", "Mechanical"), reference = .) %>%
  head(., n = 15L)
```
...
For convenience, this data frame of engineering CIP codes and program names has been bundled with midfieldr to use vignettes. 

To access the data frame, just loaf midfieldr and assign the data to a variable. 

```{r}
library(midfieldr)
df <- case_program_group
df
```
