...
We combine race and sex into a single variable using `str_c()`  from the stringr package. This is necessary to create the multiway data struture required by `multiway_order()`.
...
We use conventional ggplot2 fuctions to create the mutliway graphs. 

By previously ordering the levels of the factors, we have structured the data so that the rows and panels of the multiway graph are ordered by the approriate medians. 
