...
- `DT[, new_col := FUN]` new column, FUN is a value or a function
- `DT[, .(new_col = .N), by = grouping_variables]` count frequency N by group
...
- *Explore program CIP codes* [(link)](explore_cip.html) illustrates search strategies using `filter_by_text()`
...
There may be students matriculating in our programs who do not appear in the terms data. To account for these students, we apply a similar process to `midfieldstudents`. 
...
- *Subset for program completion feasibility*  [(link)](feasible_completion.html) describes how completion feasibility is determined  
...
- *Explore multiway data* [(link)](explore_multiway.html) describes multiway data and graphs in detail 
