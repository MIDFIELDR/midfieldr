...
The number of students has dropped to `r nrow(enrollees)`. Our approach to determining completion feasibility using `subset_feasible()` is described in the *Filter for program completion feasibility* vignette [(link)](feasible_completion.html). 
...
Multiway data and graphs and `prepare_multiway()` are described in more detail in the *Explore multiway data* vignette [(link)](explore_multiway.html).
