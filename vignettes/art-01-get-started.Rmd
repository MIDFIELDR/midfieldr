...
The `keep_text` argument gives the character string to be matched; the optional  `select` argument indicates columns to select from `cip`. 
...
```
is not as concise as possible using data.table syntax, but it provides clarity for the less experienced R user. To continue, 
...
We update the IDs a number of times, so we extract them from the data table as a character vector.  Syntax note: `DT[, unique(id)]` is the data.table equivalent to `unique(DT$id)` in base R. 
...
We use `filter_degree_seeking()` to limit our research to students in the `student` data base as these are the degree-seeking students. The default database is `midfieldstudents`.

```{r}
# limit study to students with matriculation data
DT <- filter_degree_seeking(DT)
