...
link-citations: yes
resource_files:
  - ../man/figures/impute-fye-mice-in-1.png
  - ../man/figures/impute-fye-mice-out-1.png
---
...
knitr::opts_chunk$set(
  ...
  out.width = "70%",
  fig.align = "center",
  fig.path = "../man/figures/impute-fye-"
)
...
You can skip doing the imputation yourself if you wish. We've done it for you and saved the results. We provide a replacement data set `exa_imputed_fye` with the ID and predicted starting major of all FYE students in the midfieldr data sets.  

We use these data in the package vignettes whenever an engineering starting major is required. You can do the same.

To access the case data, just load midfieldr and assign `exa_imputed_fye` to a variable. To distinguish the starting major from other 6-digit CIP codes used in this work, the imputed variable is `start` instead of `cip6`. 
...
You may skip the rest of the vignette and simply incorporate the imputed data `exa_imputed_fye` in your analysis if the details of imputation are not important to you at this time. 
...
## Data set: `exa_imputed_fye`

Construct the data set after imputation, keeping ID and starting major only. Reformat the start major as a character. The data frame, `exa_imputed_fye`, is the data mentioned previously provided with the midfieldr package if you prefer not to do your own imputation.

```{r}
exa_imputed_fye <- mice::complete(mi_output) %>%
  select(id, cip6) %>%
...
Total number of FYE students in `midfieldstudents` is `r dim(exa_imputed_fye)[1]`. 
...
```{r echo = FALSE, eval = FALSE}
# using mice
# md.pattern(exa_imputed_fye, plot = TRUE)
```

```{r mice-out, message = FALSE}
vis_dat(exa_imputed_fye, palette = "cb_safe")
```
