...
columns_we_want <- c("start", "cip6name", "N")
fye_start_summary <- fye_start_summary[order(-N), ..columns_we_want]
fye_start_summary
...
matric_programs <- filter_by_id(midfieldstudents, 
                                    keep_id = rep_ever, 
                                    keep_col = c("id", "cip6"))
# examine the result
matric_programs
...
all.equal(rep_start, start_programs)
...
mi_data <- fye_mi_prep(midfieldstudents, midfieldterms)
...
#  mi_start <- mi_start[, ..columns_we_want]
#  
...
## -----------------------------------------------------------------------------
#  mi_mids <- mice::mice(data = mi_data,
#                        method = mi_method,
