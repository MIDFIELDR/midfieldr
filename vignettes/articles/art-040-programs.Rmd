...
knitr::opts_chunk$set(fig.path = "figures/art-040-")
...
```{r child = paste0(path_to_rmd, "/define-program.Rmd")}
```

```{r child = paste0(path_to_rmd, "/define-cip.Rmd")}
```

```{r child = paste0(path_to_rmd, "/define-cip6.Rmd")}
```
...
library("midfieldr")
df <- filter_cip_rows(cip, "^41")
n41 <- nrow(df)
n4102 <- nrow(filter_cip_rows(df, "^4102"))
n4103 <- nrow(filter_cip_rows(df, "^4103"))
name41 <- unique(df[, cip2name])

df24 <- filter_cip_rows(cip, "^24")
n24 <- nrow(df24)
name24 <- unique(df24[, cip2name])

df51 <- filter_cip_rows(cip, "^51")
n51 <- nrow(df51)
name51 <- unique(df51[, cip2name])

df1313 <- filter_cip_rows(cip, "^1313")
n1313 <- nrow(df1313)
...
*Loads with midfieldr.* &nbsp; Prepared data, adapted from [@NCES:2010]. The data dictionary is documented in `?cip`. 
...
- **`pattern`** &nbsp; Character vector of search text for retaining rows, not case-sensitive. 

- **`negate`** &nbsp; Logical (default FALSE). If TRUE, returns rows that do not match the search pattern. 
...
*Output.* &nbsp; Subset of `cip` with rows matching elements of `pattern`. Additional subsetting if optional arguments specified. Examples follow. 


## Using a keyword search

Filtering the CIP data for all programs containing the word "engineering" yields `r nrow(filter_cip_rows(cip, "engineering"))` observations. 
...
If the 6-digit codes are entered as integers, they are coerced to strings for the search. 
...
Specifying 4-digit codes yields a data frame all 6-digit programs containing the 4-digit string. We use the regular expression notation `^` to match the start of the line. 
...
[&#9665; Degree seeking](art-030-degree-seeking.html) &nbsp;&nbsp; <a href="#top">&#9650; top of page</a> &nbsp;&nbsp; [Alternative CIPs &#9655;](art-041-alternative-cip.html) 
