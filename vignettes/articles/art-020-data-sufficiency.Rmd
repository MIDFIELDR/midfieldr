...
```{r child = paste0(path_to_rmd, "/define-data-range.Rmd")}
```

```{r child = paste0(path_to_rmd, "/define-timely-completion-term.Rmd")}
```

```{r child = paste0(path_to_rmd, "/define-data-sufficiency-criterion.Rmd")}
```
...
- **`midfield_rec`** &nbsp; Data frame of student-level term observations keyed by student ID. Default is `term`. Required variables (columns) are `mcid`, `term`, and `level`.
...
- **`midfield_rec`** &nbsp; Data frame of student-level term observations keyed by student ID. Default is `term`. Required variables are `mcid`, `institution`, and `term`.
...
Similar to the details described in the previous section, `data_sufficiency()` accepts [Alternate source names] and uses [Silent overwriting] when existing columns have the same name as one of the added columns. 
...
[&#9665; Planning a workflow](art-010-planning.html) &nbsp;&nbsp; <a href="#top">&#9650; top of page</a> &nbsp;&nbsp; [Degree seeking &#9655;](art-030-degree-seeking.html) 
