...
- **`midf_table`** &nbsp; Data frame of student-level term observations keyed by student ID. Default is `term`. Required variables (columns) are `mcid`, `term`, and `level`.
...
- **`entry_level`** &nbsp; Student level (01 Freshman, 02 Sophomore, etc.) in their initial term.
...
- **`midf_table`** &nbsp; Data frame of student-level term observations keyed by student ID. Default is `term`. Required variables are `mcid`, `institution`, and `term`.
