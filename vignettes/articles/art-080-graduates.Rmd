...
knitr::opts_chunk$set(fig.path = "figures/art-080-")
...
```{r child = paste0(path_to_rmd, "/define-degree-seeking.Rmd")}
```
...
```{r child = paste0(path_to_rmd, "/define-timely-completion-criterion.Rmd")}
```
...
```{r child = paste0(path_to_rmd, "/define-graduates.Rmd")}
```
...
```{r child = paste0(path_to_rmd, "/note-baseline-mcid.Rmd")}
```
...
The input data frame must have columns for ID and the timely completion term. We use `timely_term()` again, though alternatively we could have retained the `timely_term` variable from previous code chunks. 
...
- **`midfield_rec`** &nbsp; Data frame of student-level degree observations keyed by student ID. Default is `degree`. Required variables are `mcid` and `term_degree`.
...
*Filter.*  &nbsp; Filter to retain observations in the term of the first degree term only. Multiple degrees in this first degree term are allowed.  
...
[&#9665; Starters](art-070-starters.html) &nbsp;&nbsp; <a href="#top">&#9650; top of page</a> &nbsp;&nbsp; [Groupings &#9655;](art-090-groupings.html) 
