...
- `filter_programs()` chooses rows of CIP data based on search terms.

Records

- `select_records()` chooses columns of student records required by
  midfieldr functions.
- `post_bacc_terms()` identifies rows of post-baccalaureate terms to
  exclude.
- `timely_term()` estimates a student’s timely completion term.
- `data_sufficiency()` identifies rows to exclude due to insufficient
  data.
- `completion_status()` determines if program completion is timely or
  late.
...
`base::str()`. The data tables are linked by `mcid`, the anonymized
student ID.
...
`select_records()` operates on student records to reduce the number of
columns to those required by other midfieldr functions plus the key or
composite key variables of the four data tables. Shown below, the
records have been reduced to no more than 5 columns required by other
midfieldr functions.
...
if any. We are not generally interested in terms beyond the first degree
term, so we use the results of this function to exclude
post-first-degree terms.
...
Using `sort_uniq()`, a midfieldr convenience function to sort unique
values of a vector, we find the possible term cluster values to be:
...
`timely_term()` adds a column indicating the last term by which a
student’s degree completion would be considered timely, generally 6
years after admission. The timely completion term is required to
determine data sufficiency as well as assessing timely completion.
...
`data_sufficiency()` adds a column that labels each row for inclusion or
exclusion based on data sufficiency near the upper and lower bounds of
an institution’s data range.
...
term. Thus we run `timely_term()` again.
...
`completion_status()` adds a column indicating completion status for
every student.
