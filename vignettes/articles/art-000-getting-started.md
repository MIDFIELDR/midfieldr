...
- `prep_fye_mice()`   for imputing the starting majors of First-Year
  Engineering (FYE) students.
...
- `post_completion_terms()` identifies post-baccalaureate terms to
  exclude.
...
`post_completion_terms()` identifies terms later than the first
baccalaureate, if any.
...
`post_completion_terms()` adds a column indicating the cluster a term
belongs to with respect to the first degree term.
...
#>  $ gpa_term           : num  3.33 2.8 3 2.84 4 3.25 2.26 2.43 2.55 2.15 ...
#>  $ gpa_cumul          : num  3.05 2.57 2.63 2.53 2.63 2.67 2.61 2.59 2.76 2.62..
#>  $ completion_term    : chr  "19913" "19903" "19903" "19903" ...
#>  $ term_relevance     : chr  "pre-completion" "pre-completion" "pre-completio"..
```

The possible term cluster values are given by,

``` r
term[, sort(unique(term_relevance), na.last = FALSE)]
#> [1] "post-completion" "pre-completion"
```
...
``` r
term <- term["pre-completion", on = "term_relevance"]
degree <- degree["pre-completion", on = "term_relevance"]

term[, c("term_relevance", "completion_term") := NULL]
degree[, c("term_relevance", "completion_term") := NULL]
