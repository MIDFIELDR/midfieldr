...
`filter_programs()` acts on the data frame assigned to its `dframe`
argument to select rows that match or partially match search strings.
Search strings are case-independent and can include regular expressions.
...
``` r
programs <- third_pass[, .(cip6name, cip6, program = "TBD")]
...
#>                                   <char> <char>  <char>
#>  1:                       Music, General 500901     TBD
#>  2: Music History, Literature and Theory 500902     TBD
#>  3:           Music Performance, General 500903     TBD
#>  4:         Music Theory and Composition 500904     TBD
#>  5:       Musicology and Ethnomusicology 500905     TBD
#>  6:                           Conducting 500906     TBD
#>  7:                      Piano and Organ 500907     TBD
#>  8:                      Voice and Opera 500908     TBD
#>  9:   Music Management and Merchandising 500909     TBD
#> 10:                   Jazz, Jazz Studies 500910     TBD
#> 11: Vn, Va, Gtr and Other Stringed Instr 500911     TBD
#> 12:                       Music Pedagogy 500912     TBD
#> 13:                     Music Technology 500913     TBD
#> 14:                          Brass Instr 500914     TBD
#> 15:                       Woodwind Instr 500915     TBD
#> 16:                     Percussion Instr 500916     TBD
#> 17:                         Music, Other 500999     TBD
```
...
- `timely_term()` yields the timely-completion terms for every student.
...
- `completion_status()` yields the completion status for every student
  passing the data sufficiency test.
...
`timely_term()` builds a data frame with one row per student, a column
for the timely completion term, and columns of supporting information.
This data frame contains the required inputs for both
`data_sufficiency()` and `timely_completion()`.
...
`data_sufficiency()` builds on the output from `timely_term()`, labels
rows to be included or excluded based on the data sufficiency finding,
and generates additional columns of supporting information.
...
`post_bacc_terms()` identifies terms later than the first baccalaureate,
if any.
...
`post_bacc_terms()` adds a column indicating the cluster a term belongs
to with respect to the first degree term.
...
`select_basic_cols()` operates on student records to reduce the number
of columns to those required by other midfieldr functions plus the key
or composite key variables of the four data tables. With a smaller
number of columns, the printout of the data frame is more readable, a
benefit when working with the data interactively.
...
`completion_status()` builds on the output from `timely_term()`, labels
rows to indicate whether a student completes a degree timely or late
compared to their timely completion term (or NA for no completion), and
includes columns for the timely term and degree term as supporting
information.
...
`prep_fye_mice()`   Conditions data for imputing the starting majors of
First-Year Engineering (FYE) students. Used when blocs of starters in
Engineering are needed and an institution has a required FYE program.
For details see [FYE proxies](art-060-fye-proxies.html).

`order_multiway()`   Conditions data for Cleveland multiway charts. The
ordering of its rows and panels is crucial to the perception of effects.
Used when data have a multiway structure. For details see [Multiway data
and charts](art-120-multiway.html).
