...
- `filter_cip_rows()` chooses rows of CIP data based on search terms.

Records

- `select_record_cols()` chooses columns of student records required by
  midfieldr functions.
...
- `add_data_sufficiency()` identifies rows to exclude due to
  insufficient data.
- `add_completion_status()` determines if program completion is timely
  or late.
...
`filter_cip_rows()` acts on the data frame assigned to its `dframe`
...
`select_record_cols()` operates on student records to reduce the number
of columns to those required by other midfieldr functions plus the key
or composite key variables of the four data tables. Shown below, the
...
`add_term_cluster()` identifies terms later than the first
baccalaureate, if any. We are not generally interested in terms beyond
...
`add_term_cluster()` adds a column indicating the cluster a term belongs
...
`add_timely_term()` adds a column indicating the last term by which a
...
`add_data_sufficiency()` adds a column that labels each row for
inclusion or exclusion based on data sufficiency near the upper and
...
term. Thus we run `add_timely_term()` again.
...
`add_completion_status()` adds a column indicating completion status for
every student.
