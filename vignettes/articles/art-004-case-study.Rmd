...
*Records.* &nbsp; Data: `student, term,` and `degree` from midfielddata, filtered for data sufficiency, restricted to degree-seeking students, and excluding post-baccalaureate records.
...
Students in the US are *undergraduates* before their first degree completion and *graduates* afterwards. Usually, we are interested in their undergraduate records only, that is, terms that occur up to and including their first program completion. In this step we work with all columns. 
...
We use `undergraduate_terms()` to label each term "undergrad" or "graduate" and add columns to the data frame to support those findings. The function applies to the  data tables having columns with term values, i.e., `term, course,` or `degree.`
...
We keep the rows labeled "undergrad." Note that we are dropping rows by values in the *term* variable without affecting the number of students in the *population.*  
...
When dealing with the full MIDFIELD research data, we typically omit rows in which $\small N_\mathrm{grad}\leq 10$, but for these practice data we illustrate the process using $\small N_\mathrm{grad}\leq 3$. 
...
To use `ggplot()`, we want the data in its original block-record form with one value column (stickiness). We can drop the unused `race` and `sex` columns. And from the table above we see we have no values for Native American students to disseminate so we can omit those rows as well. 
