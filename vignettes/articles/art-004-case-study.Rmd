...
In this study we present a complete case with a focus on our process and its rationale. We leave the midfieldr function details to subsequent articles. 
...
These data are in block-record form with three key columns (the grouping variables), one value column (stickiness), and thus one row per value. 
...
Transform the data to row-record form. The key column `{people}` remains in place, the `{program}` column yields the new key columns `{Civil, Electrical, etc.}`, and the values in the new columns are taken from the `{stickiness}` column. The `fill` argument replaces missing values with NA.  
...
To use `ggplot()`, we want the data in its original block-record form with one value column (stickiness). 
