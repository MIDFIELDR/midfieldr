...
How to work with longitudinal student data using midfieldr, focusing on the overall  *process* rather than software details. The work is organized in three major topics: 
...
However a student finds their way to a program (just entering college, transferring, or switching majors), the program has an opportunity to see the student through to graduation. Stickiness is the fraction of students a program entices to "stick to" it through completion    [@Ohland+Orr+others:2012]. 
...
For this first section of the work, we (optionally) use `select_basic_cols()` to subset the data tables, retaining only those variables needed by  midfieldr functions. 
...
We operate on this output with `data_sufficiency()` to identify records that pass or fail the data sufficiency test and add columns to the data frame to support those findings.
...
We use an inner join, matching on `mcid,` to subset the data tables to retain records for this population only.  
...
We are generally not interested in data past the first degree term, so we identify and exclude post-first-degree terms in the source data frames. The procedure applies to the data tables  having columns of term values `{term$term, course$term_course, degree$term_degree}.`
...
Every term value occurs before or after a student completes their first degree. We use `post_completion_terms()` to label each term "before" or "after" relative to a student's first completion (if any) and add columns to the data frame to support those findings.
...
We keep the rows labeled "before." Note that we are dropping rows by values in the *term* variable without affecting the number of students in the *population.*  
...
With this step, we conclude the development of what we are calling our `_baseline` tables, filtered for data sufficiency and  degree-seeking and excluding post-first-degree terms.  
...
Here we begin the procedures that transform our baseline records and population into case-specific programs, blocs, metrics, and groupings. In this section we to search the CIP data table for the 6-digit codes for our programs. The `cip` dataset loads with midfieldr. 
...
*Summary check.* &nbsp; Numbers of students by completion status. In this case, all students completed, thus no NAs.
...
We now have the bloc of timely graduates required by our metric. We add a `bloc` variable with the value "grad" and ensure we have unique rows.
...
We now have the bloc of students ever enrolled in our programs required by our metric. We add a `bloc` variable with the value "ever" and ensure we have unique rows.
...
We have presented a complete study, from a sample of registrar's data to graphically comparing a quantitative metric grouped by people and programs. 



This study illustrates our process but the results cannot be used for drawing inferences about people or programs. The data in midfielddata are for *practice*, not *research*. 
