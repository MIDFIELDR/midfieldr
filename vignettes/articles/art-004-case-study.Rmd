...
We are not generally interested in terms beyond the first degree term, so we identify and exclude terms later than the first degree term in all the source data frames. The procedure applies only to `term` and `degree` (and, if we needed it,  `course`) because they contain columns with term data. 
