...
`look_at()` is a midfieldr convenience function that wraps `base::str()` using our preferred arguments. The data tables are linked by `mcid`, the anonymized student ID. 
...
As we prepare the data, we copy our "source" material under separate names (and locations in memory) at a couple of key points.   
...
`sort_uniq()` is a convenience function that wraps `base::sort(unique())` using our preferred arguments. The possible term cluster values are given by, 
...
*Data sufficiency* is an assessment whether a student record lies sufficiently within their institution's data range to unambiguously assess their completion status and if so include them in the study population. 
...
- `sort_uniq()` wraps base `sort(unique())` with our preferred arguments. 
