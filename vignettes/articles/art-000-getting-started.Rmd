...
- `post_completion_terms()` identifies post-baccalaureate terms to exclude.
...
`post_completion_terms()` identifies terms later than the first baccalaureate, if any.  
...
`post_completion_terms()` adds a column indicating the cluster a term belongs to with respect to the first degree term.
...
```{r}
term[, sort(unique(before_or_after), na.last = FALSE)]
```

We filter to exclude all terms labeled "post-first-degree" and drop the temporary columns. 

```{r}
term <- term["before", on = "before_or_after"]
degree <- degree["before", on = "before_or_after"]

term[, c("before_or_after", "first_degree_term") := NULL]
degree[, c("before_or_after", "first_degree_term") := NULL]
