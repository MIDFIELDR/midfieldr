...
`filter_programs()` searches `cip` for string patterns. Searching for "civil engineering"  yields programs in Engineering that we want and some in Engineering Technology that we do not. 
...
The working data frames `student, term,` and `degree` should always be present in our computing environment so we can take advantage of midfieldr default argument values. For example, `post_completion_terms()` accesses the `degree` table to do its work. If `degree` is in the environment, the following lines yield the same results:
...
`post_completion_terms()` adds a column of labels indicating that a term belongs to one of three clusters: terms that are prior to, equal to, or subsequent to the student's first degree term. 
...
`data_sufficiency()` (which requires the `timely_term` column) adds a column of labels indicating that a student ID should be included (or excluded) because for that student, the institution's data range satisfies (or does not satisfy) the data sufficiency criteria.
