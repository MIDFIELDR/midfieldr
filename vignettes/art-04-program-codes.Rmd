...
- `keep_text` character vector of search keep_texts for retaining rows, not case-sensitive 
- `drop_text` (optional) character vector of search keep_texts for dropping rows
...
As seen in the previous examples, the default setting for `select` returns all columns. We can select columns by name, 
...
The default setting for `unique_row` is TRUE and removes duplicate rows after subsetting is complete.
...
# filter_text()
filter_text(data = cip, keep_text = "engineering")
filter_text(cip, "civil engineering", drop_text = "technology")
...
# optional arguments
filter_text(cip, "^54", select = c("cip4", "cip4name"))
filter_text(cip, "^54", 
            select = c("cip4", "cip4name"), 
            unique_row = TRUE)
```
