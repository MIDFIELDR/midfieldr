...
midfieldr is designed to work with data from MIDFIELD [@ohland:midfield:2023] or any other database with a similar structure. A sample of these data (with 98,000 students) is provided in [midfielddata](https://midfieldr.github.io/midfielddata/), an R data package you can install from GitHub. 
...
We illustrate usage with a `r nrow(midfieldr::toy_student)`-student sample that loads with midfieldr. These "toy" data frames---`toy_student, toy_term,` and `toy_degree`---have the same structure as the data frames `student, term,` and `degree` in midfielddata that we use in package articles. 
