...
unpack_data <- function(x = data_bits, bit, div = 1, digits = 0) {
  x <- x %>% 
  	filter(name == bit) %>% 
  	select(data) %>% 
  	unlist() %>% 
  	unname()
  x <- round(x / div, digits)
}

obs_cip  <- unpack_data(bit = "obs_cip")
var_cip  <- unpack_data(bit = "var_cip")
size_cip <- unpack_data(bit = "size_cip", div =  1e+3)

obs_students  <- unpack_data(bit = "obs_students")
var_students  <- unpack_data(bit = "var_students")
size_students <- unpack_data(bit = "size_students", div =  1e+6)
...
size_degrees <- unpack_data(bit = "size_degrees", div =  1e+6)

obs_terms  <- unpack_data(bit = "obs_terms")
var_terms  <- unpack_data(bit = "var_terms")
size_terms <- unpack_data(bit = "size_terms", div =  1e+6)

obs_courses  <- unpack_data(bit = "obs_courses", div =  1e+6, digits = 1)
var_courses  <- unpack_data(bit = "var_courses")
size_courses <- unpack_data(bit = "size_courses", div =  1e+6)
```
...
A stratified sample of the MIDFIELD database is accessible using midfieldr. The sample comprises demographic, term, course, and degree information for `r obs_students` undergraduate students from `r year_min` to `r year_max`. Because of their size, the sample datasets are provided in separate data packages.
...
```{r graph-stickiness, fig.asp = 1.6/1.6}
