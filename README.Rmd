...
The midfielddata package  contains four data sets that are proportionate stratified random samples of 12 institutions in the larger MIDFIELD data sets. The sampling strata are institution, cip4 (the first four digits of the 6-digit CIP code), transfer status, race/ethnicity, and sex. Contains data for `r obs_students` undergraduates at `r n_inst` institutions from `r year_limits[1]`--`r year_limits[2]` in four data sets: `midfieldstudents`, `midfieldcourses`, `midfieldterms`, and `midfielddegrees`. 
...
If you are an R novice and need an introduction to the R environment, good introductory tutorials are available. You might start with: 
