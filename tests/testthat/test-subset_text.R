...
test_that("Non-character columns ignored", {
  ...
  u4$logic <- TRUE
  y4 <- subset_text(data = u4, keep_text = "^50091")
  y4$cip2 <- as.character(y4$cip2)
  y5 <- subset_text(data = music_cip, keep_text = "^50091")
  ...
  u6 <- copy(u4)
  u6$logic[1:5] <- FALSE
  y6 <- subset_text(data = u6, keep_text = "TRUE")
  y7 <- subset_text(data = u6, keep_text = "^12$")
})
test_that("Class of data frame is preserved", {
  u0 <- copy(cip)
  y0 <- subset_text(data = u0, keep_text = "^5009")
  # data.frame
  u1 <- as.data.frame(u0)
  y1 <- as.data.frame(y0)
  expect_setequal(
    class(subset_text(data = u1, keep_text = "^5009")),
    class(y1)
  )
  # data.table
  u2 <- copy(u0)
  y2 <- copy(y0)
  expect_setequal(
    class(subset_text(data = u2, keep_text = "^5009")),
    class(y2)
  )
  # tibble
  ...
  expect_setequal(
    class(subset_text(data = u3, keep_text = "^5009")),
    class(y3)
  )
})
