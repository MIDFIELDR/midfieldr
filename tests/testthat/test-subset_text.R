...
music_cip <- subset_text(cip, keep_text = "^5009")
...
test_that("Non-character columns ignored", {
  ...
  u4$logic <- TRUE
  y4 <- subset_text(dframe = u4, keep_text = "^50091")
  y4$cip2 <- as.character(y4$cip2)
  y5 <- subset_text(dframe = music_cip, keep_text = "^50091")
  ...
  u6 <- copy(u4)
  u6$logic[1:5] <- FALSE
  y6 <- subset_text(dframe = u6, keep_text = "TRUE")
  y7 <- subset_text(dframe = u6, keep_text = "^12$")
})
test_that("Class of data frame is preserved", {
  u0 <- copy(cip)
  y0 <- subset_text(dframe = u0, keep_text = "^5009")
  # data.frame
  u1 <- as.data.frame(u0)
  y1 <- as.data.frame(y0)
  expect_setequal(
    class(subset_text(dframe = u1, keep_text = "^5009")),
    class(y1)
  )
  # data.table
  u2 <- copy(u0)
  y2 <- copy(y0)
  expect_setequal(
    class(subset_text(dframe = u2, keep_text = "^5009")),
    class(y2)
  )
  # tibble
  ...
  expect_setequal(
    class(subset_text(dframe = u3, keep_text = "^5009")),
    class(y3)
  )
})
...
test_that("Data returned unaltered if keep and drop are NULL", {
  expect_equal(
    subset_text(music_cip, keep_text = NULL, drop_text = NULL),
    music_cip,
  )
})
test_that("Error if result is empty", {
  expect_error(
    subset_text(cip, keep_text = music_codes, drop_text = music_codes),
    "No CIPs satisfy the search criteria"
  )
  expect_error(
    subset_text(cip, keep_text = "enginerring"),
    "No CIPs satisfy the search criteria"
  )
})
test_that("Message if some keep_texts not found", {
  ...
  expect_message(
    subset_text(cip, keep_text = c(music_codes, not_find)),
