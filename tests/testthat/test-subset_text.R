context("subset_text")

# get_my_path() for data in the testing directory
# ctrl-shift-L to load internal functions

music_cip <- subset_text(cip, pattern = "^5009")
music_codes <- music_cip$cip6


test_that("Non-character columns ignored", {
  u4 <- copy(music_cip)
  u4$cip2 <- as.numeric(u4$cip2)
  u4$row_num <- as.numeric(rownames(u4))
  u4$logic <- TRUE
  y4 <- subset_text(dframe = u4, pattern = "^50091")
  y4$cip2 <- as.character(y4$cip2)
  y5 <- subset_text(dframe = music_cip, pattern = "^50091")
  y5$row_num <- y4$row_num
  y5$logic <- TRUE
  expect_equal(y4, y5)

  u6 <- copy(u4)
  u6$logic[1:5] <- FALSE
  y6 <- subset_text(dframe = u6, pattern = "TRUE")
  y7 <- subset_text(dframe = u6, pattern = "^12$")
})
test_that("Class of data frame is preserved", {
  u0 <- copy(cip)
  y0 <- subset_text(dframe = u0, pattern = "^5009")
  # data.frame
  u1 <- as.data.frame(u0)
  y1 <- as.data.frame(y0)
  expect_setequal(
    class(subset_text(dframe = u1, pattern = "^5009")),
    class(y1)
  )
  # data.table
  u2 <- copy(u0)
  y2 <- copy(y0)
  expect_setequal(
    class(subset_text(dframe = u2, pattern = "^5009")),
    class(y2)
  )
  # tibble
  u3 <- as.data.frame(copy(u0))
  y3 <- as.data.frame(copy(y0))
  data.table::setattr(u3, "class", c("tbl", "tbl_df", "data.frame"))
  data.table::setattr(y3, "class", c("tbl", "tbl_df", "data.frame"))
  expect_setequal(
    class(subset_text(dframe = u3, pattern = "^5009")),
    class(y3)
  )
})
# test_that("Pipe correctly passes the dframe argument", {
#   expect_equivalent(
#     cip %>% subset_text(pattern = c("^1407", "^1410")),
#     subset_text(cip, pattern = c("^1407", "^1410"))
#   )
# })
test_that("Argument order works correctly", {
  expect_equal(
    subset_text(cip, pattern = c("^1407", "^1410")),
    subset_text(cip, c("^1407", "^1410"))
  )
})
test_that("Data returned unaltered if keep and drop are NULL", {
  expect_equal(
    subset_text(music_cip, pattern = NULL, drop_text = NULL),
    music_cip,
  )
})
test_that("Error if result is empty", {
  expect_error(
    subset_text(cip, pattern = music_codes, drop_text = music_codes),
    "No CIPs satisfy the search criteria"
  )
  expect_error(
    subset_text(cip, pattern = "enginerring"),
    "No CIPs satisfy the search criteria"
  )
})
test_that("Message if some patterns not found", {
  not_find <- c("Bogus", "111111")
  expect_message(
    subset_text(cip, pattern = c(music_codes, not_find)),
    "Can't find these terms: Bogus, 111111"
  )
})
