...
music_cip <- filter_by_text(data = cip, keep_any = "^5009")
music_codes <- music_cip$cip6

test_that("Class of data frame is preserved", {
  u0 <- cip
  y0 <- filter_by_text(data = u0, keep_any = "^5009")
  # data.frame
  u1 <- as.data.frame(u0)
  y1 <- as.data.frame(y0)
  expect_setequal(
    class(filter_by_text(data = u1, keep_any = "^5009")),
    class(y1)
  )
  # data.table
  ...
  expect_setequal(
    class(filter_by_text(data = u2, keep_any = "^5009")),
    class(y2)
  )
  # tibble
  u3 <- as.data.frame(u0)
  ...
  expect_setequal(
    class(filter_by_text(data = u3, keep_any = "^5009")),
    class(y3)
  )
})
test_that("Pipe correctly passes the data argument", {
  expect_equivalent(
    cip %>% filter_by_text(keep_any = c("^1407", "^1410")),
...
test_that("Data returned unaltered if keep and drop are NULL", {
  expect_equal(
    filter_by_text(music_cip, keep_any = NULL, drop_any = NULL),
    music_cip,
  )
})
test_that("Error if result is empty", {
  expect_error(
    filter_by_text(cip, keep_any = music_codes, drop_any = music_codes),
    "No CIPs satisfy the search criteria"
  )
  expect_error(
    filter_by_text(cip, keep_any = "enginerring"),
    "No CIPs satisfy the search criteria"
  )
})
test_that("Message if some patterns not found", {
  ...
  expect_message(
    filter_by_text(cip, keep_any = c(music_codes, not_find)),
