...
var_course <- '\\item{`course`}{Character. Course name, e.g., "Astrophysics
        III", "Calculus For Social Science And Business", "Corp Financial
        Rprtng 1", "Environmental Sanitation II", "Fitness and Wellness",
...
var_bloc <- "\\item{`bloc`}{Character. A group of student-level data dealt
        with as a unit, e.g., starters, students ever-enrolled, graduates, etc.}"
...
var_discipline_midfield <- '\\item{`discipline_midfield`}{Character. A variable
        for grouping courses by academic discipline assigned by the pre-2023
        MIDFIELD data curator, e.g., "Anthropology", "Business", "Computer
...
var_faculty_rank <- '\\item{`faculty_rank`}{Character. The academic rank of the
        person teaching the course, e.g., "Assistant Professor", "Associate
        Professor", "Graduate Assistant", "Visiting Faculty", etc.}'
...
var_high_school <- '\\item{`high_school`}{Character. Code for the last high
        school attended before admission (or NA), e.g., "060075", "210512",
        "431800", "502195", etc.}'
...
var_section <- '\\item{`section`}{Character. Course section identifier, from
        one to four characters, e.g., "1", "2", "01", "14", "001", "040", "785",
...
var_term_degree <- "\\item{`term_degree`}{Character. Academic year and term
        in which a student completes their program, encoded `YYYYT.`}"
