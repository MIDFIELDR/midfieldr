...
condition_multiway <- function(dframe,
  ...
  if (order_by == "percent"){
    ...
    if (!is.null(param_col)) {warning("Unused argument 'param_col'.")}
  }
  ...
  if (!detail) {DT <- DT[, original_columns, with = FALSE]}
  DT[]
}
# --------------------------------------------------------------------------
# internal functions
...
order_by_function <- function(dframe,
  ...
  if(order_by == "sum")    {f <- sum}
  if(order_by == "median") {f <- stats::median}
  if(order_by == "mean")   {f <- mean}
