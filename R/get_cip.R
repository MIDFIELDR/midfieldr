...
#' matching or partially matching patterns.
#'
...
#'
#' \code{get_cip()} wraps the \code{\link{filter_char_frame}} operation.
...
get_cip <- function(data = NULL, keep_any = NULL, ..., drop_any = NULL) {
  ...
  # do the work
  data.table::setDF(data)
  data <- filter_char_frame(data = data,
                         keep_any = keep_any,
                         drop_any = drop_any)
  setDF(data)
  ...
  if (length(not_found) > 0 ){
    message(paste("Can't find these terms:",
